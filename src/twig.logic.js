...
module.exports = function (Twig) {
  ...
  Twig.logic.definitions = [
    ...
    },
    {
      ...
      parse(token, context, continue_chain) {
                // Parse expression
        let output = [],
          len,
          index = 0,
          ...
          loop = function (key, value) {
            const inner_context = Twig.ChildContext(context);
        ...
        if (this.blocks[token.block] === undefined || isImported || hasParent || context.loop || token.overwrite) {
          if (token.expression) {
            promise = Twig.expression.parseAsync.apply(this, [token.output, context])
                        .then(value => Twig.expression.parseAsync.apply(that, [{
                          type: Twig.expression.type.string,
                          value,
                        }, context]));
          } else {
          ...
          promise = promise.then((block_output) => {
            if (isImported) {
    ...
    },
    {
      ...
      parse(token, context, chain) {
        ...
        return promise
                .then(() => Twig.expression.parseAsync.apply(that, [token.stack, context]))
                .then((file) => {
                  if (file instanceof Twig.Template) {
                    template = file;
                  } else {
    ...
    },
    {
      ...
      parse(token, context, chain) {
        ...
        return Twig.parseAsync.apply(this, [token.output, context])
                .then((unfiltered) => {
                  let // A regular expression to find closing and opening tags with spaces between them
    ...
    },
    {
      ...
      parse(token, context, chain) {
        ...
        return Twig.expression.parseAsync.apply(this, [token.stack, context])
                .then(file => that.importFile(file || token.expression))
                .then((template) => {
    ...
    },
    {
      ...
      compile(token) {
        ...
        for (let i = 0; i < macroExpressions.length; i++) {
          ...
          const macroMatch = res.match(/^([a-zA-Z0-9_]+)\s+as\s+([a-zA-Z0-9_]+)$/);
          if (macroMatch) {
      ...
      parse(token, context, chain) {
        ...
        if (token.expression !== '_self') {
          promise = Twig.expression.parseAsync.apply(this, [token.stack, context])
                    .then(file => that.importFile(file || token.expression))
                    .then(template => template.renderAsync({}, { output: 'macros' }));
        }
    ...
    },
    {
      ...
      parse(token, context, chain) {
        ...
        return promise.then(() => Twig.expression.parseAsync.apply(that, [token.stack, innerContext]))
                .then((file) => {
                  if (file instanceof Twig.Template) {
                    template = file;
                  } else {
                  ...
                  return Twig.parseAsync.apply(that, [token.output, innerContext])
                    .then(() =>
                       ...
                       template.renderAsync(innerContext, { blocks: that.blocks }));
                })
                .then(output => ({
                  chain,
                  output,
  ...
  Twig.logic.compile = function (raw_token) {
    let expression = raw_token.value.trim(),
